...
class loader_text:
    ...
    def prepare(self,input,opt):
        self.text ={
            "train":load_file("DyCR-Net-main/input/prepared_clean/train_text"),
            "test":load_file("DyCR-Net-main/input/prepared_clean/test_text"),
            "valid":load_file("DyCR-Net-main/input/prepared_clean/valid_text")
        }
        if "len" not in opt:
            opt["len"]=100
        self.len=opt["len"]
